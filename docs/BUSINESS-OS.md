...
A dedicated **OpenMuse Business OS Web** OAuth client now exists in the owner's existing internal Google Cloud project. Gmail and Calendar APIs are enabled. The client credentials are configured on Oracle, but the owner has not yet granted the requested read scopes; Google is therefore **configured, not connected**. For an Internal Workspace app, [Google does not require](https://developers.google.com/workspace/guides/configure-oauth-consent) those scopes to be prelisted in Data Access before consent. The existing OAuth client was not changed. The Google consent screen still uses that project's existing name. Xero's OpenMuse app form has been prepared in the developer portal, but the owner has not yet accepted its developer terms or created the app; no direct Xero client is configured on Oracle.
