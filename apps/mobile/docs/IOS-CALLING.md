...
The native module keeps the most recent 64 call-stage records in `Library/Application Support/OpenMuse/voice-diagnostics.json` inside the app container. Each record contains only a timestamp, call UUID, fixed stage name, microphone permission and CallKit audio activation state. The ring records native answer/audio events and JavaScript microphone, offer and session setup stages. It contains no audio, transcript, SDP, credentials or push tokens, and is protected until the first unlock after reboot. Recording failures do not fail a call. A normal app launch does not create this file; it appears after a call-stage event.

For a paired development iPhone, copy this file with `xcrun devicectl device copy from`, using the `appDataContainer` domain and this app's bundle ID. Keep exported diagnostics in ignored private artifacts. Correlate the call UUID with the authenticated Oracle call/session record and the owner's observation; a recorded answer or prepared SDP does not prove working audio. The diagnostic build installed on 24 September retained the existing workspace sign-in, but its next call attempt is still pending.
